...
describe('Value objects', () => {
    test('Decorator order mix up', () => {
        ...
        expect(MyString.from('abc')._logicalTypes).toEqual(['my-type']);
        ...
        // derived string with explicit name
        {
            ...
            expect(MySubString.from('Abc')._logicalTypes).toEqual(['my-type', 'my-sub-type']);
        }
        ...
        // derived string with implicit name (derived from class name)
        {
            ...
            expect(() => MySubString.from('abc')).toThrow();
            expect(MySubString.from('Abc').value).toBe('Abc');
            expect(MySubString.from('Abc')._logicalTypes).toEqual(['my-type', 'my-sub-string']);
        }
        ...
        // derived string with no name
        {
            ...
            expect(() => MySubString.from('abc')).toThrow();
            expect(MySubString.from('Abc').value).toBe('Abc');
            expect(MySubString.from('Abc')._logicalTypes).toEqual(['my-type']);
        }
    });
    ...
    test('int', () => {
        ...
        expect(IntValue.fromCanonical(IntCanonical.from(12, ['int'])).value).toBe(12);
        expect(() => IntValue.fromCanonical(FloatCanonical.from(12, ['int']))).toThrow(ValidationError);
    ...
    test('float', () => {
        ...
        expect(FloatValue.fromCanonical(FloatCanonical.from(12.5)).value).toBeCloseTo(12.5, 5);
        // "x" is considered a sub-type of just "" so this is ok.
        expect(FloatValue.fromCanonical(FloatCanonical.from(12.5, ['x'])).value).toBeCloseTo(12.5, 5);
        expect(() => FloatValue.fromCanonical(IntCanonical.from(12))).toThrow(ValidationError);
        expect(() => FloatValue.fromCanonical(IntCanonical.from(12, ['x']))).toThrow(ValidationError);
    ...
    test('boolean', () => {
        ...
        expect(BoolValue.fromCanonical(BoolCanonical.from(true, [])).value).toBe(true);
        expect(() => BoolValue.fromCanonical(IntCanonical.from(1))).toThrow(ValidationError);
    ...
    test('moment', () => {
        ...
        expect(MomentValue.fromCanonical(MomentCanonical.from(DATE)).value.toISOString()).toBe(DATE.toISOString());
        expect(() => MomentValue.fromCanonical(IntCanonical.from(1))).toThrow(ValidationError);
        ...
        expect(MomentValue.from(DATE).ms).toBe(DATE.valueOf());
        expect(MomentValue.fromMilliseconds(DATE.valueOf()).ms).toBe(DATE.valueOf());
    });
