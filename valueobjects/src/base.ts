...
export function validation<T>(validator: (value: T) => string | boolean | void, description?: string) {
    ...
    return function (constructor: Function): void {
        ...
        if (!validators) {
            validators = [...Reflect.getMetadata(VALIDATORS, constructor.prototype) as ValidatorFunc[] | undefined ?? [], validatorFunc];
...
export class Value {
    ...
    public get _logicalTypes(): CanonicalLogicalTypes { throw new Error('Not implemented') }

    public equals(_other: unknown): boolean { throw new Error('Not implemented') }
...
export function toValueClass<T extends ValueClassLike<TValue>, TValue extends Value = T extends ValueClassLike<infer X> ? X : never>(v: T): ValueClass<TValue> {
    // eslint-disable-next-line @typescript-eslint/ban-types
